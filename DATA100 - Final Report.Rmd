...
0. NOTES
Inner join is used through out this project in order to remove entries with NA. If an entry has NA value(s), the entry is flawed and should not be included in the models.
...
#ERMOVE REMOVE REMVOE RMEVOE MREOVE MREOVE MREVME OREMVEOV MOER EMO REMOVE THIS
```
...
```
Observation: 
Firstly, all countries in the Northern Hemisphere have more than 50% of their population have access to the Internet while this variable varies significantly among the countries in the Southern Hemisphere. Generally, Health expenditure of Northern countries are higher than that of Southern ones. Despite this difference, many countries whose population's access to the Internet is spend proportionally as much of their GDP on the Health sector. Upon further observation, we can see that the corellations between these 2 variables are different in each hemisphere. In the north, they strongly, positively correlated. In the south, however, there is little correlation. If we assume that the percentage of the population that has access to the Internet impacts or is impacted by health expenditure, it would imply that there are other factors which holds significant sway on the Southern countries that does not affect these numbers in the Northern ones. 
Correlation coefficient = 0.27: Globally, there is a weak positive correllation between these variables. 
...
```
Observation: At first glance, for democracy score from 0 to 3.5, the percentage of population with internet access is moderately, positively correlated. However, from 3.5 to about 4.8, it is moderately, negatively correlated. From 4.8 onward, this percentage increases rapidly as democracy score increases. Also, we can clearly see that the higher the democracy score, the lower is the range percentage and vice versa.
...
Observation:
a. The scatter plot
At first glance, countries with higher percentage of population with internet access have higher age expectancy. Not only that, similar to the previous graph, as the percentage of population who has internet access increases, the range of the age expectancy get drastically more narrow. For example, at close to 1.00, most countries' age expectancy lie between 75 and 85, while at 0.25, it varies from above 50 to close to 80. Upon another look, we can see that the upper echelon of both variables are mostly occupied by Northern Hemisphere countries.

b. The boxplot:
The drastic difference in sizes of these 2 boxes is due to the fact that there are many more countries in the Southern hemisphere than in the Northern one. Just as stated in the scatterplot, the age expectancy of the Global South is much higher than GLobal North. The age expectancy of the majority of countries in the South lies between about 64 and 77, while that of the North lies between 78 and 82. 
...
```
Observations:
As you can see from the graph above, as the GDP per capita increases, the number of internet users increases exponentially, but when the GDP per capita surpasses 50000, the number of internet users starts to decrease. When we observe the smooth graph, we can see that this decrease is due to a large variance in data caused by outliers. Thus, as the GDP per capita increases, we oberve a generally small internet usage because these outliers are small countries that produced more capital than required for the country.
```{r}
...
```
Observation: It is very clear that the percentage of population with internet in Northern countries tend to be much higher than the Southern ones. Also, this percentage varies greatly across the globe. There are countries whose this value is close to 0 while many others are close to 1.
